"""
Post-Training Quantization of NAFNet-w32.

Variants measured:
  - fp32          (GPU reference)        — all 5 test sets (reused from baseline CSV)
  - fp16          (GPU, model.half())    — Rain100H/L/Test100
  - dynamic_int8  (CPU, quantize_dynamic) — Rain100H/L/Test100
  - static_int8   (CPU, FX graph mode PTQ, x86 backend, calibrated on
                   100 random Rain13K train crops) — Rain100H/L/Test100

NAFNet is the cleanest of the three baselines for PTQ:
  - No einops, no attention — pure Conv2d + SimpleGate + LayerNorm2d.
  - LayerNorm2d uses a custom autograd Function; we mark it as a
    non-traceable leaf so FX doesn't try to trace through .apply().
  - check_image_size() uses shape-dependent F.pad; we replace it with
    identity for tracing and reintroduce the 16-multiple padding in
    the forward wrapper used at eval time.

CPU-bound variants are evaluated only on the 3 smaller test sets
(Test1200/Test2800 would take hours on CPU).

Writes results/baselines/tables/nafnet_w32_quantization.csv.
...
CKPT = PROJECT_ROOT / "pretrained" / "nafnet_w32_deraining.pth"
CKPT_DIR = PROJECT_ROOT / "experiments" / "quantization" / "checkpoints"
CSV_PATH = PROJECT_ROOT / "results" / "baselines" / "tables" / "nafnet_w32_quantization.csv"
...
def pad_to_multiple(x: torch.Tensor, m: int = PADDER_SIZE) -> tuple:
    """Pad H/W up to a multiple of m. Returns (padded, H, W)."""
    _, _, h, w = x.shape
    mod_h = (m - h % m) % m
    mod_w = (m - w % m) % m
...
def eval_quality(forward_fn, device: str, testset_path: Path, label: str,
    ...
    """Evaluate on a test set. If manual_pad=True, pad inputs to PADDER_SIZE
    multiples before forward_fn and crop output back (needed when the model's
    own check_image_size has been bypassed for FX tracing)."""
...
# ---------------------------------------------------------------------------
def run_fp32() -> dict:
    """FP32 reference. Reuses PSNR/SSIM/LPIPS from the baselines CSV
    (already computed across all 5 test sets) and remeasures latency."""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = load_model("nafnet_w32", str(CKPT), device)
    forward = lambda x: model(x)
    lat = measure_latency(forward, device)

    baseline_csv = PROJECT_ROOT / "results" / "baselines" / "tables" / "nafnet_w32_results.csv"
    baseline = {}
...
def run_fp16() -> dict:
    ...
    fp16_path = CKPT_DIR / "nafnet_w32_fp16.pth"
...
def run_dynamic_int8() -> dict:
    ...
    ckpt_path = CKPT_DIR / "nafnet_w32_dynamic_int8.pth"
    try:
    ...
    after = _count_modules(qmodel)
    swapped = {k: after[k] for k in after if "Dynamic" in k or "Quantized" in k}
    note_lines = [
        f"Dynamic INT8 state_dict saved ({size_mb:.2f} MB).",
        f"Modules swapped by quantize_dynamic: {swapped or 'NONE — NAFNet has no nn.Linear layers; quantize_dynamic is a no-op for conv-only models.'}",
    ]
...
def run_static_int8() -> dict:
    ...
    # Bypass check_image_size for tracing — we'll pad manually at eval time.
    ...
    mod_counts = _count_modules(qmodel)
    ...
    ckpt_path = CKPT_DIR / ("nafnet_w32_static_int8_conv_only.pth"
                             if used_fallback else "nafnet_w32_static_int8.pth")
    try:
        torch.save(qmodel.state_dict(), ckpt_path)
    except Exception as exc:
...
def print_summary(rows: list):
    ...
    fp32_psnr_3 = np.nanmean([r["psnr"] for r in fp32_rows if r["dataset"] in TESTSETS_CPU]) if fp32_rows else float("nan")
    ...
    for var, rs in by_var.items():
        psnrs = [r["psnr"] for r in rs if not np.isnan(r["psnr"]) and r["dataset"] in TESTSETS_CPU]
        avg_psnr = np.mean(psnrs) if psnrs else float("nan")
        size = rs[0]["model_size_mb"]
        lat = rs[0]["latency_ms"]
        dev = rs[0]["device"]
        dp = avg_psnr - fp32_psnr_3
